...
class _PostcardListScreenState extends State<PostcardListScreen> {
    ...
    return GestureDetector(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final stampLeft = w * t.stampX / 100 - 14 * t.stampScale / 100;
          final stampTop  = h * t.stampY / 100 - 16 * t.stampScale / 100;
          final postmarkLeft = w * t.postmarkX / 100 - 21 * t.postmarkScale / 100;
          final postmarkTop  = h * t.postmarkY / 100 - 21 * t.postmarkScale / 100;
          return Container(
            ...
            child: ClipRRect(
              ...
              child: Stack(children: [
                ...
                // Postmark — template-driven position
                if (postmark != null)
                  Positioned(left: postmarkLeft, top: postmarkTop, child: _buildTilePostmark(postmark, t.postmarkScale)),
                // Stamp — template-driven position
                if (stamp != null)
                  Positioned(left: stampLeft, top: stampTop, child: _buildTileStamp(stamp, t.stampScale)),
                // Inner border
                Positioned.fill(
                  child: Container(
  ...
  Widget _buildTileStamp(PostcardStamp stamp, double scale) {
    final sw = 28 * scale / 100;
    final sh = 32 * scale / 100;
    return stamp.imageUrl != null && stamp.imageUrl!.isNotEmpty
        ? ClipRRect(borderRadius: BorderRadius.circular(2), child: Image.network(ApiService.imageUrl(stamp.imageUrl), width: sw, height: sh, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Center(child: Text(stamp.emoji, style: TextStyle(fontSize: 12 * scale / 100)))))
        : Text(stamp.emoji, style: TextStyle(fontSize: 14 * scale / 100));
  }

  Widget _buildTilePostmark(PostcardPostmark postmark, double scale) {
    final size = 42 * scale / 100;
    if (postmark.imageUrl != null && postmark.imageUrl!.isNotEmpty) {
      return Image.network(ApiService.imageUrl(postmark.imageUrl), width: size, height: size, fit: BoxFit.contain, errorBuilder: (_, __, ___) => _buildTilePostmarkCircle(postmark, size));
    }
    return _buildTilePostmarkCircle(postmark, size);
  }

  Widget _buildTilePostmarkCircle(PostcardPostmark postmark, double size) {
    return Container(
