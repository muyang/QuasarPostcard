...
function drawTextField(ctx, placeholder, name, color, fontFamily, fontSize,
  xPct, yPct, boxW, boxH, borderColor, borderWidth, bgColor, bgOpacity, w, h, isExport) {
  var cx = w * xPct / 100;
  var cy = h * yPct / 100;
  var x = cx - boxW / 2;
  var y = cy - boxH / 2;
  ...
  // When exporting and field is empty, skip placeholder rendering entirely
  if (isExport && isEmpty) return;
...
// Layer 9: Message
function drawMessage(ctx, message, tpl, w, h, customColor, customFont, customSize, isExport) {
  var x = w * tpl.messageX / 100 + 6;
  var y = h * tpl.messageY / 100 + 6;
  var maxW = w * tpl.messageW / 100 - 12;
  var maxH = tpl.messageH - 12;
  var text = message || '';
  var color = customColor || tpl.messageColor;
  var font = customFont || tpl.messageFont;
  var size = customSize !== undefined ? customSize : tpl.messageSize;
...
function exportHiRes(design) {
  ...
  return drawPostcard(offscreen, design, { scale: constants.EXPORT_SCALE, imgSize: null, isExport: true }).then(function() {
    return new Promise(function(resolve, reject) {
      wx.canvasToTempFilePath({
        canvas: offscreen,
        x: 0, y: 0,
